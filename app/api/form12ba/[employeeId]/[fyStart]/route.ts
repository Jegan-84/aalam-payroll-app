...
import { buildForm12BABuffer } from '@/lib/pdf/build-form12ba'
...
export async function GET(_req: Request, { params }: { params: PP }) {
  ...
  const result = await buildForm12BABuffer(employeeId, fyStart)
  if (!result) return new NextResponse('No perquisites recorded for this FY', { status: 404 })
